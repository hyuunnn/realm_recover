...
class RealmRecover(FileHeader):
    ...
    def parse_objects(self, tree_root_offset):
        ...
        obj = ObjectParser(self._buf, tree_root_offset).parse_object()
        tableInformation = ObjectParser(self._buf, obj[0]).parse_object()[2:]
        table_array_offsets = ObjectParser(self._buf, obj[1]).parse_object()[2:]

        tables = []
        for offset in table_array_offsets:
            tableArray = ObjectParser(self._buf, offset).parse_object()
            ...
            aa = ObjectParser(self._buf, table_schema_offset).parse_object()
            tableSchema = [
                ObjectParser(self._buf, aa[0]).parse_column_type(),  # column_type offset
                ObjectParser(self._buf, aa[1]).parse_column_name(),  # column_name offset
            ]
            dataStorage = ObjectParser(self._buf, data_storage_offset, recursive=1).parse_object()
